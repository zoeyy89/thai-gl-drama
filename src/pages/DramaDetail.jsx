...
function DramaDetail() {
  ...
  const toggleFavorite = () => {
    ...
    if (isFav) {
      localStorage.setItem('favorites', JSON.stringify(current.filter(favId => favId !== drama.id)))
    } else {
  ...
  return (
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: '32px 0' }}>
      ...
      {/* 回上頁 */}
      <Link to="/dramas" style={{ color: '#326fc3', textDecoration: 'none', fontSize: '14px' }}>
        ← 回劇集列表
      </Link>
      ...
      <div style={{ margin: '24px 0 16px' }}>
        ...
        <h2 style={{ margin: '0', color: '#888', fontSize: '18px', fontWeight: 'normal' }}>
          {drama.englishTitle}
        </h2>
      </div>
      ...
      {drama.cp.length > 0 && (
        <div style={{
          ...
          <p style={{ margin: '0 0 8px', color: '#132c56', fontWeight: 'bold' }}>主演 CP</p>
          {drama.cp.map((c, i) => (
            <p key={i} style={{ margin: '0', fontSize: '15px' }}>
              <Link to={`/cp/${c.name}`} style={{ color: '#326fc3', fontWeight: 'bold' }}>
                {c.name}
              </Link>
              　{c.actor1} × {c.actor2}
            </p>
      ...
      {/* 預告片 */}
      {drama.trailer && (
        <a href={drama.trailer} target="_blank" rel="noreferrer" style={{
          display: 'inline-block',
          background: '#132c56',
          color: 'white',
          borderRadius: '8px',
