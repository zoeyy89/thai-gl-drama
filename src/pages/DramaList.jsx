...
function DramaList() {
  ...
  const allYears = [...new Set(dramas.map(d => d.year))].sort((a, b) => b - a)
  ...
  const filteredDramas = dramas.filter(drama => {
    ...
    if (filterTags.length > 0 && !filterTags.every(tag => drama.tags.includes(tag))) return false
    return true
  })
      ...
      <div style={{ display: 'flex', gap: '12px', margin: '16px 0' }}>
        {sortedYears.map(year => (
          <a key={year} href={`#year-${year}`}
            style={{ color: '#326fc3', textDecoration: 'none', fontWeight: 'bold' }}>
            {year}
          </a>
        ))}
      </div>
            ...
            {groupedByYear[year].map(drama => (
              <div key={drama.id} style={{
                ...
                <Link to={`/dramas/${drama.id}`} style={{ textDecoration: 'none' }}>
                  <h3 style={{ margin: '0 0 8px 0', color: '#132c56', fontSize: '18px' }}>
                    {drama.fullTitle}
                  </h3>
                </Link>
                <p style={{ margin: '0 0 6px 0', color: '#888', fontSize: '14px' }}>
                  {drama.year} | {drama.episodes > 0 ? `${drama.episodes}集` : '集數待補'} | {drama.status}
                </p>
                <p style={{ margin: '0 0 8px 0', color: '#444', fontSize: '15px' }}>
                  {drama.description}
                </p>
                <p style={{ margin: '0 0 6px 0', fontSize: '14px' }}>
                  ⭐ {drama.rating > 0 ? drama.rating : '暫無評分'}
                </p>
                <p style={{ margin: '0 0 6px 0', fontSize: '13px', color: '#326fc3' }}>
                  {drama.tags.join(' · ')}
                </p>
                {drama.cp.length > 0 && (
                  <p style={{ margin: '0', fontSize: '13px', color: '#888' }}>
                    👥 {drama.cp.map((c, i) => (
                      <span key={i}>
                        <Link to={`/cp/${c.name}`} style={{ color: '#326fc3' }}>{c.name}</Link>
                        （{c.actor1} × {c.actor2}）
                      </span>
                    ))}
                  </p>
